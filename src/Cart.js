                                        ...
                                        {saved.amount > 1 && (
                                            <button onClick={() => {
                                                const updatedItems = savedItems.map(item =>
                                                    item.$id === saved.$id ? { ...item, amount: item.amount - 1 } : item
                                                );
                        ...
                        <div className='bill'>
                            ...
                            <div className='row delivery'>
                                ...
                                <select value={pay} onChange={(e) => { setPay(e.target.value) }}>
                                    <option key={0} value='расчётный счёт'>Расчётный счёт</option>
                                    <option key={1} value='Наличными'>Наличными</option>
                                </select>
                            </div>
                            ...
                            <button
                                onClick={() => {
                                    axios.post(URL_API, {
                                        chat_id: CHAT_ID,
                                        parse_mode: 'html',
                                        text: `Заявка на заказ, номер телефона: ${phone}, товары: ${savedItems.map(item => `${item.amount} штук: ${item.title} `)}, нужна ли доставка: ${delivery}, количество дней: ${days}, дата: ${date}, тип расчёта: ${pay}`
                                    })
